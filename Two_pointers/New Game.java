...
public class Main {
    ...
    static void testcase() {
        ...
        for (int i = 0; i < n; i++) {
            int x = in.nextInt();
            fr.put(x, fr.containsKey(x) ? fr.get(x) + 1 : 1);
            if (fr.get(x) == 1) {
                nums.add(x);
            }
        }
        ...
        // dbg(nums);
        int curAns = fr.get(nums.get(0)), ans = curAns;
        int l = 0, r = 0;
        for (l = 0, r = 1; r < nums.size(); r++) {
            if (nums.get(r) == nums.get(r - 1) + 1) {
                curAns += fr.get(nums.get(r));
                if (r - l + 1 <= k)
                    ans = max(ans, curAns);
            } else {
                if (r - l + 1 <= k)
